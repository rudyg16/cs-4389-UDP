#!/usr/bin/env bash
# Wrapper to run the low-and-slow UDP trickle from the attacker namespace.
set -euo pipefail
...
echo "[*] Launching low-and-slow traffic from $NS_ATTACKER -> $TARGET_IP:$TARGET_PORT (pps=$PPS duration=${DURATION}s)"
ip netns exec "$NS_ATTACKER" python3 "$REPO_ROOT/udp-shield-testbed/low_and_slow.py" \
